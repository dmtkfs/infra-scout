...
def parse_json_arg(val: str):
    """Accept either a JSON string literal or a path to a JSON file."""
    p = Path(val)
    if p.exists():
...
def build_summary(rows):
    ...
    for name, status, reason in rows:
        parts.append(
            f"{name.ljust(width_name)}  {fmt_status(status).ljust(width_status)}  {reason}"
        )
    return "\n".join(parts)
...
def explain_deps(status, ctx):
    ...
    if status in ("WARN", "FAIL") and majors:
        out.append(
            "• One or more dependencies jumped a major version. That can include breaking changes."
        )
        out.append(
            "• Common causes: unpinned versions, lockfile drift, or an intentional upgrade not yet vetted."
        )
        out.append(
            "• Check next: review changelogs, run tests locally/CI, and decide whether to pin or adopt the new major. If expected, update the snapshot."
        )
    elif changed or added or removed:
        out.append("• Dependencies changed compared to the last snapshot.")
        out.append(
            "• Common causes: new feature work, transitive resolution changes, or environment differences."
        )
        out.append(
            "• Check next: confirm intent, ensure reproducibility, and update the snapshot once validated."
        )
    else:
...
def explain_secrets(status, findings):
    ...
    if status == "FAIL" and findings:
        out.append(
            "• Lines in the selected diff resemble credentials or keys (pattern/entropy based)."
        )
        out.append(
            "• Could be false positives (random IDs) or real secrets (e.g., cloud/API keys)."
        )
        out.append(
            "• Check next: confirm validity, rotate if real, store in a secrets manager; allowlist safe test tokens if necessary."
        )
        out.append(
            "• Example: `High-entropy` ≈ statistically key-like; `AWS Access Key` ≈ matches AKIA* format."
        )
    else:
...
def explain_image(status, counts):
    ...
    if status == "FAIL" and crit > 0:
        ...
        out.append(
            "• Causes: outdated base image, unpatched packages, or known issues in dependencies."
        )
        out.append(
            "• Check next: bump base image, apply fixed versions, or rebuild regularly from updated bases."
        )
    elif (hi + med) > 0:
        out.append(
            "• Non-critical CVEs present; triage based on exploitability and exposure."
        )
        out.append(
            "• Check next: prioritize HIGH items; schedule rebuilds/patch windows."
        )
    else:
...
def explain_canary(status, failed_count):
    ...
    if status == "FAIL":
        out.append(
            "• One or more endpoints didn’t meet expected status and/or latency thresholds."
        )
        out.append(
            "• Causes may include downtime, auth/permissions, network/firewall rules, wrong expected code, or strict thresholds."
        )
        out.append(
            "• Check next: confirm endpoint health and expected code; review SLOs and adjust p50/p95 thresholds to match reality."
        )
    else:
...
def deps_check(
    ...
    lines = [title]
    if added:
        lines.append("• Added: " + ", ".join(f"{n}=={v or '?'}" for n, v in added))
    if removed:
        lines.append("• Removed: " + ", ".join(f"{n}=={v or '?'}" for n, v in removed))
    if upgraded:
        lines.append(
            "• Changed: "
            + ", ".join(f"{n} {o or '?'} → {v or '?'}" for n, o, v in upgraded)
        )
    if majors:
...
DEFAULT_PATTERNS = [
    ...
    (
        "AWS Secret (base64-ish)",
        r"(?i)aws(.{0,20})?(secret|key).{0,5}[=:]\s*[A-Za-z0-9/+=]{32,}",
    ),
        ...
        for i, line in enumerate(content.splitlines(), start=1):
            ...
            for name, pat in DEFAULT_PATTERNS:
                if re.search(pat, line):
                    findings.append(
                        (str(f.relative_to(repo_dir)), i, name, line.strip()[:80])
                    )
    lines = [title]
    if findings:
...
def parse_trivy_json(txt: str):
    ...
    for res in results:
        ...
        for v in vulns:
            ...
            if len(top) < 5:
                top.append(
                    f"{v.get('VulnerabilityID','?')} ({sev}) {v.get('PkgName','?')} {v.get('InstalledVersion','?')} → {v.get('FixedVersion','-')}"
                )
...
def image_check(image: str = None, sample_json: Path = None, title="[Image Scan]"):
    ...
    if image:
        proc = run_cmd(
            ["trivy", "image", "--quiet", "--format", "json", image], capture=True
        )
    ...
    if status == "FAIL":
        reason = f"{crit} CRITICAL"
    lines = [
        f"{title} Counts → CRITICAL:{crit} HIGH:{hi} MED:{med} LOW:{low} UNK:{unk}"
    ]
    if top:
...
def canary_check(targets, probes=3, timeout=3.0, title="[Canary]"):
    ...
    for t in targets:
        ...
        lines.append(
            f"  - latency p50={p50}ms p95={p95}ms  thresholds p50≤{max_p50} p95≤{max_p95}"
        )
        lines.append(f"  - status_ok={ok_status}  latency_ok={ok_latency}")

        per_target.append(
            {"url": url, "ok_status": ok_status, "ok_latency": ok_latency}
        )

    status = "OK" if failed == 0 else "FAIL"
    reason = (
        "All checks passed" if status == "OK" else f"{failed} endpoint(s) failed checks"
    )
...
def demo_setup(workdir: Path):
    ...
    (workdir / ".cache/dep_last.json").write_text(
        json.dumps({"requests": "2.27.0", "urllib3": "1.26.18"}, indent=2),
        encoding="utf-8",
    )
...
def build_parser():
    ...
    def add_out(sp, name):
        sp.add_argument(
            "--out",
            default=None,
            help=f"Write report to path (default: {REPORT_DIR}/<timestamp>_{name}.txt)",
        )
    ...
    sp6 = sub.add_parser(
        "demo", help="Full self-contained demo (fails each check), then cleans up"
    )
...
def main():
    ...
    if args.cmd == "deps":
        ...
        s, reason, block, ctx = deps_check(
            manifest, snapshot, fail_on_major=args.fail_on_major, title=title
        )
        ...
        if used_demo:
            ...
            cleanup = "[Cleanup]\nRemoved temporary workspace: " + str(
                workdir.resolve()
            )
        ...
        if used_demo:
            ...
            cleanup = "[Cleanup]\nRemoved temporary workspace: " + str(repo.resolve())
            progress("Cleanup complete.")
        text = (
            spaced(
                header,
        ...
        s, reason, block, ctx = image_check(
            args.image,
            Path(args.sample_json) if args.sample_json else None,
            title=title,
        )
        ...
        text = (
            spaced(header, summary, spaced(block, explain), "Overall: " + fmt_status(s))
            + "\n"
        )
        write_and_print(out, text)
        sys.exit(0 if s in ("OK", "WARN") else 1)
        ...
        if args.targets:
            ...
            mode = "auto-fallback"
            progress(
                "No targets given → starting a local canary server on 127.0.0.1:9999"
            )
        ...
        s, reason, block, ctx = canary_check(
            targets, probes=args.probes, timeout=args.timeout, title=title
        )
        ...
        if not (manifest.exists() and snapshot.exists() and (repo / ".git").exists()):
            mode = "auto-fallback"
            progress(
                "Run: components missing → using a temporary demo workspace for a full showcase."
            )
        ...
        s1, r1, b1, c1 = deps_check(
            manifest, snapshot, fail_on_major=args.fail_on_major, title=title_deps
        )
        s2, r2, b2, c2 = secrets_check(repo, diff_range, title=title_secrets)
        s3, r3, b3, c3 = image_check(
            args.image,
            Path(args.sample_json) if args.sample_json else None,
            title=title_image,
        )
        s4, r4, b4, c4 = canary_check(
            targets, probes=args.probes, timeout=args.timeout, title=title_canary
        )
        ...
        if used_workdir:
            ...
            cleanup = (
                ...
                + str(used_workdir.resolve())
            )
            progress("Cleanup complete.")

        overall = (
            "OK"
            if all(s in ("OK", "WARN") for s in [s1, s2, s3, s4])
            and "FAIL" not in [s1, s2, s3, s4]
            else "FAIL"
        )
        text = (
            spaced(
                ...
                sections,
                cleanup,
                "Overall: " + fmt_status(overall),
            )
            + "\n"
        )
        progress("Writing report → " + str(out))
        write_and_print(out, text)
        ...
        targets = [
            ...
            {
                "url": "http://127.0.0.1:9998/healthz",
                "expect_status": 200,
                "max_ms_p50": 300,
                "max_ms_p95": 800,
            },
        ...
        s1, r1, b1, c1 = deps_check(
            ...
            workdir / ".cache/dep_last.json",
            fail_on_major=True,
            title=title_deps,
        )
        s2, r2, b2, c2 = secrets_check(workdir, diff_range, title=title_secrets)
        s3, r3, b3, c3 = image_check(
            None, workdir / "trivy_sample.json", title=title_image
        )
        s4, r4, b4, c4 = canary_check(
            targets, probes=3, timeout=2.0, title=title_canary
        )
        ...
        if not getattr(args, "keep", False):
            shutil.rmtree(workdir, ignore_errors=True)
            cleanup += "\nRemoved temporary workspace: " + str(workdir.resolve())
        ...
        text = (
            spaced(header, summary, sections, cleanup, "Overall: " + fmt_status("FAIL"))
            + "\n"
        )
        progress("Writing report → " + str(out))
        write_and_print(out, text)
